...
from model.sefm_model import SeFM
...
parser.add_argument('--epochs', type=int, default=500, help='total number of epoch')
parser.add_argument('--image-size', type=int, default=512, help='image crop size')
...
def build_model(opt):
    ...
    return model, discriminitor, optimizer, optimizer_dis, loss, scheduler
...
def train(epoch, model, discriminitor, train_loader, optimizer, optimizer_dis, train_loss, opt):
...
# =====================restoration network=============================
        ...
        losses = train_loss['l1'](output, target) + 0.3 * train_loss['bce'](dis_out_o, type)
        losses.backward()
        optimizer_rest.step()
...
def main(opt):
    ...
    model, discriminitor, optimizer, optimizer_dis, loss, scheduler = build_model(opt)
    scheduler.step()
    if opt.val:
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        epoch_train_loss = train(epoch, model, discriminitor, train_loader, optimizer, optimizer_dis, loss, opt)
